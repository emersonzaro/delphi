...
begin
  ...
  if ((Trim(edSalario.Text)) = '') or ((Trim(edTotalVendas.Text)) = '') then
    ShowMessage('Campos salário e total de vendas não podem ser vazios')
  else
  begin
    ...
    begin
      total := total;
      ShowMessage('Salário final: R$ ' + FloatToStr(total) + ' - Não houve prêmio')
    end

    else if (totalvendas > 1000) and (totalvendas <= 5000) then
    begin
      total := (salario + premio500);
      ShowMessage('Salário final: R$ ' + FloatToStr(total) + ' - Prêmio: R$ ' +
